...
async function executeRealOrdersFixed() {
  console.log('🚀 EXECUTING REAL ORDERS (FIXED FOR INDIA DELTA EXCHANGE)\n');
    ...
    if (monitorResult.positions && monitorResult.positions.length > 0) {
      ...
      for (const follower of followers) {
        ...
        for (const position of openPositions) {
          ...
          if (copySize > 0) {
            // Convert to contract size (minimum 1 contract)
            const contractSize = Math.max(1, Math.ceil(copySize * 10)); // 1 contract = 10 POL
            
            console.log(`   🔧 Converting to contract size: ${copySize} → ${contractSize} contracts`);

            // Execute real order on follower's Delta Exchange account
            const orderResult = await executeDeltaOrderFixed(
              follower.api_key,
              follower.api_secret,
              position.product_id,
              contractSize,
            ...
            if (orderResult.success) {
              console.log(`✅ Real order executed for ${follower.follower_name}: ${symbol} ${side} ${contractSize} contracts`);
...
// Function to execute order on Delta Exchange (FIXED FOR INDIA API)
async function executeDeltaOrderFixed(apiKey, apiSecret, productId, size, side, price, apiUrl) {
  try {
    const timestamp = Math.floor(Date.now() / 1000);
    const path = '/v2/orders';
    
    // Fixed order format for India Delta Exchange
    const orderData = {
      product_id: productId,
      size: size, // Integer value (contract size)
      side: side,
      order_type: 'market_order' // Correct order type
    };
...
executeRealOrdersFixed().catch(console.error); 