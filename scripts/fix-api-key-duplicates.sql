-- Fix API key duplicate prevention
-- Run this in your Supabase SQL Editor

-- 1. First, let's check what constraints currently exist
SELECT 'Current Constraints on broker_accounts' as info;
SELECT 
  constraint_name,
  constraint_type,
  table_name
FROM information_schema.table_constraints 
WHERE table_name = 'broker_accounts' 
AND constraint_type = 'UNIQUE';

-- 2. Remove the existing API key constraint if it exists (it might be too restrictive)
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'unique_user_api_key' 
    AND table_name = 'broker_accounts'
  ) THEN
    ALTER TABLE broker_accounts DROP CONSTRAINT unique_user_api_key;
  END IF;
END $$;

-- 3. Add a more specific constraint for API key + secret combination
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'unique_user_api_combination' 
    AND table_name = 'broker_accounts'
  ) THEN
    ALTER TABLE broker_accounts DROP CONSTRAINT unique_user_api_combination;
  END IF;
END $$;

ALTER TABLE broker_accounts 
ADD CONSTRAINT unique_user_api_combination 
UNIQUE (user_id, api_key, api_secret);

-- 4. Update the validation function to properly check API key duplicates
CREATE OR REPLACE FUNCTION validate_broker_account()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if user already has an account with the same broker and account_uid
  IF EXISTS (
    SELECT 1 FROM broker_accounts 
    WHERE user_id = NEW.user_id 
    AND broker_name = NEW.broker_name 
    AND account_uid = NEW.account_uid
    AND id != NEW.id
  ) THEN
    RAISE EXCEPTION 'You already have an account with this broker and account ID';
  END IF;
  
  -- Check if user already has an account with the same account_name
  IF EXISTS (
    SELECT 1 FROM broker_accounts 
    WHERE user_id = NEW.user_id 
    AND account_name = NEW.account_name
    AND id != NEW.id
  ) THEN
    RAISE EXCEPTION 'You already have an account with this name';
  END IF;
  
  -- Check if user already has an account with the same API key + secret combination
  IF EXISTS (
    SELECT 1 FROM broker_accounts 
    WHERE user_id = NEW.user_id 
    AND api_key = NEW.api_key 
    AND api_secret = NEW.api_secret
    AND id != NEW.id
  ) THEN
    RAISE EXCEPTION 'You already have an account with this API key and secret combination';
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 5. Create helper functions for checking duplicates
CREATE OR REPLACE FUNCTION can_add_broker_account(
  user_uuid uuid,
  broker_name_input text,
  account_uid_input text,
  account_name_input text,
  api_key_input text,
  api_secret_input text
)
RETURNS TABLE (
  can_add boolean,
  reason text
) AS $$
BEGIN
  -- Check if account with same broker and account_uid exists
  IF EXISTS (
    SELECT 1 FROM broker_accounts 
    WHERE user_id = user_uuid 
    AND broker_name = broker_name_input 
    AND account_uid = account_uid_input
  ) THEN
    RETURN QUERY SELECT false, 'Account with this broker and account ID already exists';
    RETURN;
  END IF;
  
  -- Check if account with same name exists
  IF EXISTS (
    SELECT 1 FROM broker_accounts 
    WHERE user_id = user_uuid 
    AND account_name = account_name_input
  ) THEN
    RETURN QUERY SELECT false, 'Account with this name already exists';
    RETURN;
  END IF;
  
  -- Check if account with same API key + secret combination exists
  IF EXISTS (
    SELECT 1 FROM broker_accounts 
    WHERE user_id = user_uuid 
    AND api_key = api_key_input 
    AND api_secret = api_secret_input
  ) THEN
    RETURN QUERY SELECT false, 'Account with this API key and secret combination already exists';
    RETURN;
  END IF;
  
  RETURN QUERY SELECT true, 'Account can be added';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 6. Create function to check if user can follow a trader
CREATE OR REPLACE FUNCTION can_follow_trader(
  follower_uuid uuid,
  trader_uuid uuid
)
RETURNS TABLE (
  can_follow boolean,
  reason text
) AS $$
BEGIN
  -- Check if user is trying to follow themselves
  IF follower_uuid = trader_uuid THEN
    RETURN QUERY SELECT false, 'You cannot follow yourself';
    RETURN;
  END IF;
  
  -- Check if user already follows this trader
  IF EXISTS (
    SELECT 1 FROM public.subscriptions 
    WHERE follower_id = follower_uuid 
    AND trader_id = trader_uuid
  ) THEN
    RETURN QUERY SELECT false, 'You are already following this trader';
    RETURN;
  END IF;
  
  RETURN QUERY SELECT true, 'You can follow this trader';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 7. Create helper functions to get user's broker accounts and subscriptions
CREATE OR REPLACE FUNCTION get_user_broker_accounts(user_uuid uuid)
RETURNS TABLE (
  id uuid,
  broker_name text,
  account_name text,
  account_uid text,
  is_active boolean,
  created_at timestamptz
) AS $$
BEGIN
  RETURN QUERY 
  SELECT 
    ba.id,
    ba.broker_name,
    ba.account_name,
    ba.account_uid,
    ba.is_active,
    ba.created_at
  FROM broker_accounts ba
  WHERE ba.user_id = user_uuid
  ORDER BY ba.created_at DESC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION get_user_subscriptions(user_uuid uuid)
RETURNS TABLE (
  id uuid,
  trader_id uuid,
  trader_name text,
  trader_email text,
  created_at timestamptz
) AS $$
BEGIN
  RETURN QUERY 
  SELECT 
    s.id,
    s.trader_id,
    u.name as trader_name,
    u.email as trader_email,
    s.created_at
  FROM public.subscriptions s
  JOIN public.users u ON s.trader_id = u.id
  WHERE s.follower_id = user_uuid
  ORDER BY s.created_at DESC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 8. Verify the updated constraints
SELECT 'Updated Constraints on broker_accounts' as info;
SELECT 
  constraint_name,
  constraint_type,
  table_name
FROM information_schema.table_constraints 
WHERE table_name = 'broker_accounts' 
AND constraint_type = 'UNIQUE'; 