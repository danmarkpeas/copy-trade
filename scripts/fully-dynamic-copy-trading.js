...
class FullyDynamicCopyTrading {
  constructor() {
    this.supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
    this.pollingInterval = 2000;
    this.lastMasterPositions = new Map();
    this.productIds = {}; // Dynamically loaded from API
    this.followers = []; // Dynamically loaded from database
    this.masterAccounts = []; // Dynamically loaded from database
    this.isRunning = false;
    this.pollCount = 0;
  }

  async initialize() {
    console.log('🚀 INITIALIZING FULLY DYNAMIC COPY TRADING SYSTEM');
    
    // Load all dynamic data
    await this.loadDynamicProductIds();
    await this.loadAllFollowers();
    await this.loadAllMasterAccounts();
    await this.initializeBackendMonitoring();
    
    console.log('✅ Fully dynamic copy trading system initialized successfully!');
  }

  async loadDynamicProductIds() {
    try {
      console.log('📡 Loading all tradable symbols from Delta Exchange API...');
      ...
      console.log('📊 Available symbols:', Object.keys(this.productIds).slice(0, 10), '...');
      
    } catch (error) {
      console.error('❌ Error loading dynamic product IDs:', error.message);
      throw error; // Don't use fallback - fail if API is unavailable
    }
  }

  async loadAllFollowers() {
    try {
      console.log('📋 Loading all active followers from database...');
      ...
      console.log(`✅ Loaded ${followers.length} active follower(s) from database`);
      
      // Verify API credentials for each follower dynamically
      ...
      console.error('❌ Error loading followers from database:', error.message);
      throw error;
    }
  }

  async loadAllMasterAccounts() {
    try {
      console.log('📋 Loading all active master accounts from database...');
      
      const { data: masterAccounts, error } = await this.supabase
      ...
      if (!masterAccounts || masterAccounts.length === 0) {
        throw new Error('No active master broker accounts found in database');
      }

      this.masterAccounts = masterAccounts;
      console.log(`✅ Loaded ${masterAccounts.length} active master account(s) from database`);
      
      // Log master account details
      masterAccounts.forEach(account => {
        console.log(`   📊 ${account.broker_name || account.account_name}: ${account.id}`);
      });
    } catch (error) {
      console.error('❌ Error loading master accounts from database:', error.message);
      throw error;
    }
  }
  ...
  async initializeBackendMonitoring() {
    try {
      ...
      // Check follower balances dynamically
      for (const follower of this.followers) {
        try {
          const balance = await this.getFollowerBalance(follower);
          console.log(`   ${follower.follower_name} balance: $${balance}`);
        } catch (error) {
          console.log(`   ${follower.follower_name} balance: Error - ${error.message}`);
        }
      }
    } catch (error) {
  ...
  async verifyFollowerCredentials(follower) {
    try {
      const balance = await this.getFollowerBalance(follower);
      return balance >= 0; // Allow zero balance for testing
    } catch (error) {
  ...
  async start() {
    console.log('🚀 STARTING FULLY DYNAMIC COPY TRADING');
    console.log(`⚡ Polling interval: ${this.pollingInterval}ms`);
    console.log(`📡 Monitoring ${this.followers.length} followers for ${this.masterAccounts.length} masters`);
    console.log(`🎯 Supporting ${Object.keys(this.productIds).length} symbols dynamically`);
  ...
  async dynamicPoll() {
    ...
    try {
      ...
      if (this.pollCount % 10 === 0) {
        ...
        console.log(`⏰ ${time} - Dynamic monitoring active... (${Object.keys(this.productIds).length} symbols, ${this.followers.length} followers)`);
      }
      ...
      console.error('❌ Error in dynamic poll:', error.message);
    }

    setTimeout(() => this.dynamicPoll(), this.pollingInterval);
  }
  ...
  async executeCopyTrades(masterTrade) {
    // Check if symbol is supported dynamically
    if (!this.productIds[masterTrade.symbol]) {
      console.log(`⚠️ Symbol ${masterTrade.symbol} not found in dynamic product IDs`);
      return;
    }

    // Execute for all active followers dynamically
    for (const follower of this.followers) {
      try {
        // Get follower's copy settings from database dynamically
        const copySize = await this.calculateDynamicCopySize(follower, masterTrade);
        if (copySize <= 0) {
          console.log(`⚠️ Insufficient balance for ${follower.follower_name} to copy ${masterTrade.symbol}`);
          continue;
        }

        const orderResult = await this.placeDynamicCopyOrder(follower, masterTrade, copySize);
  ...
  async calculateDynamicCopySize(follower, masterTrade) {
    try {
      // Get follower's balance dynamically
      const balance = await this.getFollowerBalance(follower);
      
      // Get follower's copy settings from database dynamically
      const copyRatio = follower.copy_ratio || 1;
      const fixedLot = follower.fixed_lot || null;
      const multiplier = follower.multiplier || 1;
      
      let baseSize;
      if (fixedLot) {
        baseSize = fixedLot; // Use fixed lot size if set
      } else {
        baseSize = masterTrade.size * copyRatio * multiplier; // Use dynamic calculation
      }
      
      // Calculate maximum possible size based on balance
      ...
      console.error(`❌ Error calculating dynamic copy size for ${follower.follower_name}:`, error.message);
      return 0;
    }
  }

  async placeDynamicCopyOrder(follower, masterTrade, copySize) {
    try {
  ...
  async closeFollowerPositions(symbol, masterPosition) {
    // Close positions for all followers dynamically
    for (const follower of this.followers) {
      try {
        ...
        for (const position of followerPositions) {
          if (Math.abs(position.size) > 0.001) {
            await this.placeDynamicCloseOrder(follower, position);
          }
        }
      } catch (error) {
  ...
  async placeDynamicCloseOrder(follower, position) {
    try {
      // Get product ID dynamically
      ...
      console.error(`❌ Error placing dynamic close order for ${follower.follower_name}:`, error.message);
      return { success: false, error: error.message };
    }
  }
  ...
  async saveCopyTrade(masterTrade, follower, copySize, status, errorMessage = null) {
    try {
      const copyTradeData = {
        ...
        follower_id: follower.user_id, // Using dynamic user_id from database
  ...
  stop() {
    console.log('\n🛑 Stopping fully dynamic copy trading system...');
    this.isRunning = false;
  }
}
...
// Start the system
async function main() {
  const system = new FullyDynamicCopyTrading();
  ...
  try {
    ...
    console.error('❌ Failed to start fully dynamic system:', error.message);
    process.exit(1);
  }
}
