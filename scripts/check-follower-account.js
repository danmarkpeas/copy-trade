...
async function checkFollowerAccount() {
  console.log('🔍 CHECKING FOLLOWER ACCOUNT WITH PROFILE UUID 57068604\n');
  ...
  try {
    // 1. Check if this profile UUID exists in broker_accounts
    console.log('📋 STEP 1: Checking Broker Account');
    const { data: brokerAccounts, error: brokerError } = await supabase
      .from('broker_accounts')
      .select('*')
      .eq('profile_id', '57068604');
    ...
    if (!brokerAccounts || brokerAccounts.length === 0) {
      console.log('❌ No broker account found with profile UUID 57068604');
    ...
    console.log('2. You should now see the test copy trade');
    ...
    console.log('❌ Error checking follower account:', error.message);
  }
}

checkFollowerAccount().catch(console.error); 