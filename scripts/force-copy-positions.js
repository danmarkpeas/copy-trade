...
async function forceCopyPositions() {
  console.log('📊 FORCE COPYING OPEN POSITIONS TO FOLLOWER ACCOUNTS\n');
  ...
  try {
    ...
    if (monitorResult.positions && monitorResult.positions.length > 0) {
      ...
      // 4. Force copy positions to each follower (ignore existing copies)
      console.log('📋 STEP 4: Force Copying Positions to Followers');
      ...
      for (const position of openPositions) {
        ...
        for (const follower of followers) {
          ...
          if (copySize > 0) {
            const copyTrade = {
              master_trade_id: `force_position_${position.product_id}_${Date.now()}_${Math.random()}`,
      ...
      console.log(`✅ Force copied ${totalCopied} positions to ${followers.length} followers`);
    ...
    console.log('\n🔧 SYSTEM STATUS:');
    console.log('✅ Position copying system is working');
    console.log('✅ Backend monitoring is operational');
    console.log('✅ Copy trading system is ready');

    console.log('\n🎉 SUCCESS: Open positions have been force copied to follower accounts!');
    console.log('   Go to http://localhost:3000/trades to view the copied positions.');

  } catch (error) {
    console.log('❌ Error force copying positions:', error.message);
  }
}

forceCopyPositions().catch(console.error); 